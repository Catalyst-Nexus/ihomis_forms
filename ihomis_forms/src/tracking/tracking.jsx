...
import {
  ...
  parseApiStatus,
  safeDisplay,
} from "./utils/chartTrackingUtils.js";
...
export default function Tracking({
  ...
  const selectedPatientLabel = useMemo(() => {
    ...
    const displayName =
      selectedPatient?.displayName || raw.patient_name || raw.patientName || "";
    const hpercode =
      raw.hpercode ||
    ...
    if (!displayName && !hpercode) {
      return "";
    }

    return `${displayName || "Patient"}${hpercode ? ` · HPER ${hpercode}` : ""}`;
  }, [selectedPatient]);
  ...
  const visibleStepIds = useMemo(() => {
    if (!currentUserId || !Object.keys(accessMap).length)
      ...
      <main className="tracking-shell">
        <header className="tracking-title-box">
          ...
          {selectedPatientLabel && (
            <small>
              Selected patient: <strong>{selectedPatientLabel}</strong>
            </small>
          )}
        </header>
