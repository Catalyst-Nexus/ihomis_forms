...
function LabUploadModule() {
  ...
  const [contextLoading, setContextLoading] = useState(
    Boolean(LAB_UPLOAD_CONTEXT_URL),
  );
        ...
        <section className="lab-grid lab-reveal">
          <form className="lab-panel lab-form" onSubmit={handleSubmit}>
            ...
            <p className="lab-file-hint">
              You can select multiple PDFs at once, add more anytime, and retry
              failed files individually.
            </p>

            {resultFiles.length ? (
              <section
                className="lab-file-queue"
                ...
                <button
                  ...
                  onClick={() => {
                    const firstPreviewIndex =
                      getFirstUploadedPreviewIndex(uploadedFiles);
                    if (firstPreviewIndex >= 0) {
                      setReviewSource("uploaded");
                      setActiveUploadedFileIndex(firstPreviewIndex);
                    }
