...
export async function fetchLabPatientCandidates({
  ...
  const serverPagination = responsePayload?.pagination;
  let nextPageAvailable = false;
  if (Number.isFinite(Number(serverPagination?.total))) {
    const total = Number(serverPagination.total);
    const currentOffset = Number(serverPagination.offset) || pageIndex * pageSize;
