...
function useLabPatientPicker({
  ...
  const pageSize = 10;
  // Request more than pageSize to compensate for deduplication
  // which can remove rows that share the same id
  const apiLimit = pageSize + 5;
  ...
  useEffect(() => {
    ...
    async function loadPatientCandidates() {
      ...
      try {
        ...
        let currentOffset = pageIndex * pageSize;
        ...
        for (let attempt = 0; attempt < maxOffsets; attempt++) {
          ...
          // Stop if we reached pageSize OR the response had no new items
          if (allCandidatesMap.size >= pageSize) {
            break;
          }

          // No more data coming
          if (addedCount === 0 || rows.length < apiLimit) {
            break;
          }
        ...
        if (Number.isFinite(Number(responsePagination?.total))) {
          ...
          nextPageAvailable = candidates.length >= pageSize;
        }
